function [simMPM,tracksSim] = simulateRandomPlusLinearMotion(imSize,numP,...
...
fracType = cumsum(fracType);
randNumber = rand(numTracks,1);
mType = 2*ones(numTracks,1); %start with confined
mType(randNumber<fracType(1)) = 1; %unconfined
mType(randNumber>=fracType(2)) = 3; %directed
    ...
    if cnf > 1
        ...
        switch trackType
            ...
            case 1 %unconfined
                ...
                %get trajectory
                xyvecTraj = brownianMotion(2,trackDiffCoef(iTrack),cnf-1,0.1);
                
            case 2 %confined
                
                %get trajectory
                xyvecTraj = brownianMotion(2,trackDiffCoef(iTrack),cnf-1,0.1,1,trackConfRad(iTrack));
            ...
            case 3 %directed
                ...
                xyvecTraj = brownianMotion(2,trackDiffCoef(iTrack),lengthSeg(1),0.1,[],[],trackVel(1,:));
                for iSeg = 2 : numSeg
                    tmpTraj = brownianMotion(2,trackDiffCoef(iTrack),lengthSeg(iSeg),0.1,[],[],trackVel(iSeg,:));
