...
%get total movie time
totalTime=length(compTracks(1).tracksFeatIndxCG);
%% Calculation
...
for iTrack = 1 : numTracks
    ...
    % calculate cluster sizes from intensities
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%Modification LRO 2018/01/17%%%%%%%%%%%%%%%%%%%%%%%%%%%
    for iAgg=1:size(aggregStateMat,1)
        % calculate the lifetime of this segment
        lifeTimeIntervalSeg= ~isnan(aggregStateMat(iAgg,:));
        % calculate the distribution of mean intensities
        meanAmplitude=mean(tracksAmp(iAgg,lifeTimeIntervalSeg))/intensityInfo(1);
        
        clusterSizeAmp=round(meanAmplitude);
        
        
        % check for each segment the cluster size from merge and split and if
        % the intensity info is giving a cluster bigger than the size given by
        % merge and split, replace this cluster size and also all that are
        % interacting with this segment
        
        % check if the maximum cluster size is bigger than these coming from
        
        clusterSizeMS=max(max(aggregStateMat(iAgg,lifeTimeIntervalSeg)));
        
        % if cluster size intensity can increase cluster size from merge and
        % split events
        
        if clusterSizeMS<clusterSizeAmp
            
            % replace the cluster size
            
            aggregStateMat(iAgg,lifeTimeIntervalSeg)=clusterSizeAmp;
            
            
        end % if intensity increase cluster size
