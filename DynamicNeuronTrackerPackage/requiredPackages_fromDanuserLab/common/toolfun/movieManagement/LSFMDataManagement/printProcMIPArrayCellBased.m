 function processRenderArray=printProcMIPArrayCellBased(processCellArray,savePath,varargin)
%% MIPSize: 
%% 
ip = inputParser;
...
for fIdx=1:numel(printFrame)
    ...
    renderCell{fIdx}=buildArray(processCellArray,frameIdx,[p.maxHeight p.maxWidth],p.MIParrangement,p.MIPSize,stripeSize,stripeColor,varargin{:});
end

processRenderArray=CachedAnimation(savePath,numel(printFrame));
for fIdx=1:numel(printFrame)
    processRenderArray.saveView(fIdx,renderCell{fIdx});
end


function render=buildArray(processCellArray,frameIdx,maxArraySize,arrangementType,mipSize,stripeSize,stripeColor,varargin)
    ip = inputParser;
    ip.KeepUnmatched = true;
    ip.addParamValue('forceHeight',false,@islogical);
    ip.addParamValue('forceWidth',false,@islogical);
...
for i=1:numel(processCellArray)
    if(~isempty(processCellArray{i}))
        if(iscell(processCellArray{i}))
            ...
            frameNb=processCellArray{i}.getFrameNb();
            mMaxXY=processCellArray{i}.loadView(min(frameNb,frameIdx));
...
% rendert=zeros(3,sum(renderPixelSizeHeight(:,1)),sum(renderPixelSizeWidth(1,:)));
% size(rendert)
